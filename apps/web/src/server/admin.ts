...
export type CourierRecord = { id: string; name: string; phone: string; trips: CourierDetailDto['trips'] }
export type AdminUserDetails = AdminUserDetailsDto
export type AdminSession = Admin2SessionDto['admin'] extends null ? never : NonNullable<Admin2SessionDto['admin']> extends never ? never : { loginAt: Date; logoutAt: Date | null; wasActive: boolean }
...
export async function getAdminUsers(filters: {
  ...
  if (filters.device && filters.device !== 'all') params.set('device', filters.device)
...
export async function getUserAuditLogs(input: {
  data: { userId: string }
}): Promise<Array<{ id: string; userName: string; action: string; timestamp: Date }>> {
  return authJson<Array<{ id: string; userName: string; action: string; timestamp: Date }>>(
    `/admin/users/${input.data.userId}/activities`,
    'GET',
  )
}
...
export async function getAdminOrders(filters: {
  ...
  if (filters.search) params.set('search', filters.search)
  if (filters.status && filters.status !== 'all') params.set('status', filters.status)
  if (filters.sortDate && filters.sortDate !== 'none') params.set('sortDate', filters.sortDate)
  if (filters.sortAmount && filters.sortAmount !== 'none') params.set('sortAmount', filters.sortAmount)
  if (filters.confirmedBy && filters.confirmedBy !== 'all') params.set('confirmedBy', filters.confirmedBy)
...
export async function getAdmin2Options(): Promise<Array<{ id: string; name: string }>> {
...
export async function getCourierOptions(): Promise<Array<{ id: string; name: string }>> {
...
export async function getOrderDetailsByRole(input: {
  ...
  return authJson<LiveOrderDto | null>(`/live/orders/${input.data.orderId}`, 'GET')
}
...
export async function addCourier(input: {
  ...
  return authJson<{ success: boolean; message?: string }>('/admin/couriers', 'POST', input)
}
...
export async function addSubAdmin(input: {
  ...
  return authJson<{ success: boolean; message?: string }>('/admin/admins', 'POST', {
    phone: input.phone,
...
export async function updateSubAdminPermissions(input: {
  ...
  await authJson<unknown>(`/admin/admins/${input.id}/permissions`, 'PATCH', input.permissions)
}

export async function subAdminLogout(input: { data: { adminId: string } }): Promise<void> {
  void input
...
export async function viewOrderNote(input: {
  ...
  return authJson<{ note: string | null }>(`/live/orders/${input.data.orderId}/note`, 'POST')
}
