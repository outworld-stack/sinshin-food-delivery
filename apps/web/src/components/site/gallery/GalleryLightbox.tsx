...
export const GalleryLightbox = memo(function GalleryLightbox({
	...
	return (
		...
		>
			...
			<div className="w-full max-w-4xl h-[70vh] rounded-3xl overflow-hidden shadow-2xl bg-black/20">
				{/* round-12 — عکس آپلودی URL واقعی؛ گرادیانت موک قدیمی هم ممکن است باشد */}
				{isRealImageUrl(src) ? (
					<img src={src} alt={alt} className="w-full h-full object-contain" />
				) : (
					<div
						className={`w-full h-full bg-linear-to-br ${src}`}
						role="img"
						aria-label={alt}
					/>
				)}
