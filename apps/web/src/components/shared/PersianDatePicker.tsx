...
interface PersianDatePickerProps {
  ...
  placeholder?: string
  /** phase-9: اتصال label فرم (a11y) — به دکمه‌ی تریگر می‌رسد */
  id?: string
}
...
export const PersianDatePicker = memo(function PersianDatePicker({
  value, onChange, placeholder = 'انتخاب تاریخ...', id,
  ...
  return (
    <div className="relative">
      ...
      {state.isOpen && (
        <>
          {/* phase-9: بک‌دراپ = دکمه‌ی واقعی (a11y) — کلیک هرجا = بستن */}
          <button
            type="button"
          ...
          <div className="absolute z-50 mt-1 w-72 bg-white dark:bg-[#2a1015] rounded-2xl border border-gray-200 dark:border-[#3a151c] shadow-xl p-4">
            ...
            <div className="grid grid-cols-7 gap-1">
              {dayCells.map((day, i) => {
                // biome-ignore lint/suspicious/noArrayIndexKey: سلول خالی تقویم — ایندکس همان شناسه است (روز واقعی با key={day})
