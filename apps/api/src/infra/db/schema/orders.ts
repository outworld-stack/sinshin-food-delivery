...
import type { CampaignId, CourierId, OrderId, ProductId, SizeId } from '#/domain/shared/brand'
...
export const orders = pgTable(
  'orders',
  {
    ...
    courierId: uuid('courier_id').$type<CourierId>().references(() => couriers.id, { onDelete: 'set null' }),
    courierSecurityEnabled: boolean('courier_security_enabled').notNull().default(false),
    courierArrivedAt: timestamp('courier_arrived_at', { withTimezone: true }),
    deliveredAt: timestamp('delivered_at', { withTimezone: true }),

    paymentStatus: varchar('payment_status', { length: 20 }).notNull().default('PENDING'),
    paymentMethod: varchar('payment_method', { length: 20 }).notNull().default('GATEWAY'),
    ...
    courierLocation: jsonb('courier_location').$type<{ lat: number; lng: number }>(),
    customerLocation: jsonb('customer_location').$type<{ lat: number; lng: number }>(),
