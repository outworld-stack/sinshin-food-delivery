...
export class LiveService {
	...
	async liveOrders(
		...
		const ownershipSql = isMainAdmin
			? sql`true`
	...
	async confirmOrder(
		...
		if (viewerRole !== "admin") {
			...
			if (!inScope)
				return { success: false, message: "این سفارش خارج از حوزه‌ی شماست" };
		}
		...
		const [updated] = await this.deps.db
			...
			.set({
				...
				internalNotePrint:
					(input.courierNote ?? "").trim().length > 0 &&
					input.notePrintOnInvoice === true,
				updatedAt: new Date(),
			})
		...
		if (!updated)
			return {
				success: false,
				message: "سفارش هم‌زمان توسط ادمین دیگری تایید شد",
			};
    ...
    `);
		...
		this.deps.hub.publish("orders:new", {
			...
			data: { id: displayId, by: adminUserId },
		});
		return { success: true };
	}
	...
	async reassignCourier(
		...
		if (row.courierArrivedAt) {
			return {
				success: false,
				message: "پیک به مغازه رسیده — امکان تغییر نیست",
			};
		}
		...
		await this.deps.db
			...
			.where(eq(orders.id, row.id));
		await this.deps.admin2.log(adminUserId, "COURIER_REASSIGN", displayId, {
			newCourierId,
		});
		await this.deps.admin2.touchActivity(adminUserId);
		this.deps.hub.publish("orders:new", {
	...
	private async toViews(
		...
		const courierIds = [
			...new Set(
				rows
					.map(({ o }) => o.courierId)
					.filter((x): x is CourierId => x !== null),
			),
		];
		const confirmerIds = [
			...new Set(
