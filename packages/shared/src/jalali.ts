...
// تقویم شمسی — هستهٔ مشترک API + وب (استخراج از web/utils/persianDate تا
// فقط یک پیاده‌سازی داشته باشیم — DRY). بر پایهٔ Intl (موتور خود
// مرورگر/Node، ECMA-402) — صفر الگوریتم دستی = صفر باگ. SSR-safe.
//
// توجه: این ماژول باید «خالص» بماند (بدون وابستگی به فرانت/بک) چون از
// هر دو اپ ایمپورت می‌شود.
...
export function gregorianToJalali(date: Date): JalaliDate {
        ...
        const get = (t: string): number =>
                Number(toEnDigits(parts.find((p) => p.type === t)?.value ?? 'NaN'))
...
export function jalaliToGregorian(j: JalaliDate): Date {
        ...
        // روزِ سال (1-based)
        const doy =
                j.month <= 7 ? (j.month - 1) * 31 + j.day : 186 + (j.month - 7) * 30 + j.day
...
export function daysInJalaliMonth(jy: number, jm: number): number {
        ...
        return Math.round((nextNowruz.getTime() - esfand1.getTime()) / 86400000) === 366
                ? 30
                : 29
}
